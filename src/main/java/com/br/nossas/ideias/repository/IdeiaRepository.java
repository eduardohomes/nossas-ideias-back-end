...
public interface IdeiaRepository extends JpaRepository<Ideia, Long> {
	...
	@Query(value = "select ide.* \n" +
            ...
            "where voto.voto = 'S'\n" +
            "and ide.situacao = 'Em Colaboração'\n" +
    ...
    @Query(value = "select ide.* " +
            ...
            "join nossaideia.favorita favo " +
            "on ide.id = favo.id_ideia " +
            "where favo.id_user = ?1 and favo.marcada = 'S' " +
            "and ide.situacao <> 'Rejeitada'", nativeQuery = true)
